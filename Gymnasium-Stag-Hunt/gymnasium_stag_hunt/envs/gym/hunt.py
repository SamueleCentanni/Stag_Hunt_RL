    ...
    def __init__(
        ...
        if obs_type == "image":
            self.observation_space = Box(
                0,
                255,
                shape=(2, grid_size[0] * TILE_SIZE, grid_size[1] * TILE_SIZE, 3), # 2 is for the two agents
                dtype=uint8,
            )
        elif obs_type == "coords":
            self.observation_space = Box(
                0, max(grid_size), shape=(2, 6 + forage_quantity * 2,), dtype=uint8 # 2 is for the two agents
            )
